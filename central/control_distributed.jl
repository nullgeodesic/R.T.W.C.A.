...
function iterate_on_pix_range(ray_bundle,img_bundle,tolerance::Real,colors,colors_freq,
    ...
    println("Elapsed time (ns): ",time_ns()-start_time)
...
function alltogethernow_RKDP(;camera_pos=[0,0,0,0],camera_dir=[0.0,0.0],speed=0.0::Real,
    ...
    #Main Loop
    chunks = Iterators.partition(1:num_pix,cld(num_pix,nworkers()))
    tasks = map(chunks) do chunk
        @spawnat :any iterate_on_pix_range(deepcopy(long_ray_matrix[chunk,:]),deepcopy(long_xyY_img[chunk]),
