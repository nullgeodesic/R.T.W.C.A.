...
@inline function calc_christoffel_udd!(ray,Γ)
    ...
    Γ[1,1,2] = r_s*(r^2 + α^2)*(r^2 - (α*cos(θ))^2) / (2*Σ^2*Δ)
    Γ[1,2,1] = Γ[1,1,2]
    Γ[1,1,3] = -r_s*α^2*r*sin(2*θ) / (2*Σ^2)
    Γ[1,3,1] = Γ[1,1,3]
    Γ[1,2,4] = r_s*α*sin(θ)^2*((α*cos(θ))^2*(α^2 - r^2) - r^2*(α^2 + 3*r^2)) / (2*Σ^2*Δ)
    Γ[1,4,2] = Γ[1,2,4]
    Γ[1,3,4] = r_s*α^3*r*sin(θ)^3*cos(θ) / Σ^2
    Γ[1,4,3] = Γ[1,3,4]
    
    #r Christoffel symbols
    Γ[2,1,1] = r_s*Δ*(r^2 - α^2*cos(θ)^2) / (2*Σ^3)
    Γ[2,1,4] = -Δ*r_s*α*sin(θ)^2*(r^2 - (α*cos(θ))^2) / (2*Σ^3)
    Γ[2,4,1] = Γ[2,1,4]
    Γ[2,2,2] = (2*r*(α*sin(θ))^2 - r_s*(r^2 - (α*cos(θ))^2)) / (2*Σ*Δ)
    Γ[2,2,3] = -α^2*sin(2*θ) / (2*Σ)
    Γ[2,3,2] = Γ[2,2,3]
    Γ[2,3,3] = -r*Δ / Σ
    Γ[2,4,4] = Δ*sin(θ)^2*(-2*r*Σ^2 + r_s*(α*sin(θ))^2*(r^2 - (α*cos(θ))^2)) / (2*Σ^3)
    
    #θ Christoffel symbols
    Γ[3,1,1] = -r_s*α^2*r*sin(2*θ) / (2*Σ^3)
    Γ[3,1,4] = r_s*α*r*(r^2 + α^2)*sin(2*θ) / (2*Σ^3)
    Γ[3,4,1] = Γ[3,1,4]
    Γ[3,2,2] = α^2*sin(2*θ) / (2*Σ*Δ)
    Γ[3,2,3] = r / Σ
    Γ[3,3,2] = Γ[3,2,3]
    Γ[3,3,3] = -α^2*sin(2*θ) / (2*Σ)
    Γ[3,4,4] = -sin(2*θ)*(A*Σ + (r^2 + α^2)*r_s*r*(α*sin(θ))^2) / (2*Σ^3)
    
    #ϕ Christoffel symbols
    Γ[4,1,2] = r_s*α*(r^2 - (α*cos(θ))^2) / (2*Σ^2*Δ)
    Γ[4,2,1] = Γ[4,1,2]
    Γ[4,1,3] = -r_s*α*r*cot(θ) / Σ^2
    Γ[4,3,1] = Γ[4,1,3]
    Γ[4,2,4] = (2*r*Σ^2 + r_s*((α^2*sin(2*θ)/2)^2 - r^2*(Σ + r^2 + α^2))) / (2*Σ^2*Δ)
    Γ[4,4,2] = (2*r*Σ^2 + r_s*((α^2*sin(2*θ)/2)^2 - r^2*(Σ + r^2 + α^2))) / (2*Σ^2*Δ)
    Γ[4,3,4] = cot(θ)*(Σ^2 + r_s*r*(α*sin(θ))^2) / Σ^2
    Γ[4,4,3] = Γ[4,3,4]
